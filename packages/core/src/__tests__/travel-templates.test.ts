...
describe('TRAVEL_TEMPLATES validation', () => {
  it('should have at least one template', () => {
    expect(TRAVEL_TEMPLATES.length).toBeGreaterThan(0);
  });

  it('should have unique IDs across all templates', () => {
    const ids = TRAVEL_TEMPLATES.map((t) => t.id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  for (const template of TRAVEL_TEMPLATES) {
...
describe('getTravelTemplateById', () => {
  it('should return the correct template for a known ID', () => {
    const template = getTravelTemplateById('hotel-room-circuit');
    expect(template).toBeDefined();
    expect(template?.id).toBe('hotel-room-circuit');
    expect(template?.name).toBe('Hotel Room Circuit');
  });

  it('should return undefined for an unknown ID', () => {
    expect(getTravelTemplateById('nonexistent-template')).toBeUndefined();
  });

  it('should return undefined for an empty string', () => {
    expect(getTravelTemplateById('')).toBeUndefined();
  });
});

describe('getTravelTemplatesByCategory', () => {
  it('should return only strength templates', () => {
    const templates = getTravelTemplatesByCategory('strength');
  ...
  it('should return only core templates', () => {
    const templates = getTravelTemplatesByCategory('core');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {
  ...
  it('should return only mobility templates', () => {
    const templates = getTravelTemplatesByCategory('mobility');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {
  ...
  it('should return only plyometric templates', () => {
    const templates = getTravelTemplatesByCategory('plyometric');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {
...
describe('getTravelTemplatesByDifficulty', () => {
  it('should return only beginner templates', () => {
    const templates = getTravelTemplatesByDifficulty('beginner');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {
  ...
  it('should return only intermediate templates', () => {
    const templates = getTravelTemplatesByDifficulty('intermediate');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {
  ...
  it('should return only advanced templates', () => {
    const templates = getTravelTemplatesByDifficulty('advanced');
    expect(templates.length).toBeGreaterThan(0);
    for (const t of templates) {

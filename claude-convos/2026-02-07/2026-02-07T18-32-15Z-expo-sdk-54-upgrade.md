...
- Add E2E tests to catch startup issues before release
...
Added Maestro E2E tests to catch runtime startup issues:
- `app-startup.yaml`: Verifies app launches and all tabs visible
- `tab-navigation.yaml`: Verifies navigation between tabs

### CI/CD Enhancements
Added E2E testing via EAS Workflows and Claude Convos enforcement:

1. **E2E Tests (EAS Workflows):** Runs Maestro tests on Expo's infrastructure
   - Triggers on PRs via GitHub integration
   - Builds APK using `e2e-test` profile
   - Faster than GitHub Actions + emulator approach

2. **Claude Convos Check (claude-convos-check.yml):** Enforces AI transparency
...
- `apps/mobile/.maestro/` - New E2E test flows
- `apps/mobile/.eas/workflows/e2e-test-android.yml` - E2E test workflow
- `.github/workflows/claude-convos-check.yml` - AI conversation logging enforcement
- `CONTRIBUTING.md` - Updated with E2E testing docs
...
- [Maestro E2E Testing](https://docs.expo.dev/eas/workflows/examples/e2e-tests/)
